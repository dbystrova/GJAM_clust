...
We get the `fit2` gjam object. This object is the same as the one from the original model, except several additional parameters. Firstly, we can plot the prior distribution of the alpha parameter and get expected number of clusters for this prior distribution of $\alpha$ (We suggest to test it, due to stochastic nature of hyperparameter tunning). $\alpha$ $\sim$ Ga$(\nu_1, \nu_2)$. Where $\nu_1$ is shape and $\nu_2$ rate parameters corresponingly in the modelList `otherpar` list in the `reductList` in  `modelList`  of gjam object `fit2`.


```{r check, include=TRUE, echo=TRUE, warning=FALSE, eval=F}
 ...
 * plot_prior  plot  the prior distribution  for $\alpha$ parameter
 * plot_all    plot for the prior and posterior  for $\alpha$ parameter
 * alpha      vector of samples from  prior  distribution of $\alpha$ parameter
 * E_k        expected number of clusters for this parameter of alpha
 

Then we need to estimate the clusters. The cluster estimation is done by the function gjam_cluster, 
which would return optimal cluster, that summarize posterior cluster distribution. We use the method proposedby 
Wade et al., 2018 where we use the clusters  
...
We can also consider applcation of the dimension reduction with type 3
We can use the variance to have less peaky distribution
